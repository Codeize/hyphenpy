...
with open(f"wd.sqlite", "rb") as file:
    time = datetime.now()
    time = time.strftime("%d-%m-%y %H%M")
    if not os.path.exists(os.path.join("bot_config/Info", "db_backups")):
        os.makedirs(os.path.join("bot_config/Info", "db_backups"))
...
class db():
    ...
    def __init__(self, verbose=True):
        ...
        conn.commit()
        print(" > Connected to WD DB") if verbose else False
    ...
    def Add_Guild(self, guild_id):
        c = conn.cursor()
        t = (guild_id,)
        c.execute("INSERT INTO guilds VALUES(?, 'off', 'watchdog', 'all', 1000000);", t)
